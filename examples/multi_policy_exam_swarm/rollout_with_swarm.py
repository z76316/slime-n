...
# num_parallel is K (samples per agent per outer prompt). Must equal
# n_samples_per_prompt in config.yaml.
SWARM_CONFIGS = {
...
async def generate_with_swarm(args, sample: Sample, sampling_params, evaluation: bool = False) -> list[Sample]:
    """Per outer prompt, dispatch to all 8 agents and collect K responses
    each. Returns 8 × K samples flat, tagged with policy_name for the
    manager's split-buffer routing."""
