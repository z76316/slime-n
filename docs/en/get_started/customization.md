...
When implementing custom functions, you'll work with the `Sample` dataclass:
...
## Example: Implementing Custom Reward

Here's a complete example of implementing a custom reward function:
...
def check_format(response: str) -> float:
    # Implement your format checking logic
    pass
```
...
## Example: Implementing Multi-Turn Generation
...
1. **Use Async Functions**: Most rollout-related functions should be async for better performance.

2. **Handle Errors Gracefully**: Wrap your custom logic in try-except blocks to prevent crashes.

3. **Log Important Information**: Use Python's logging module to track your custom logic.

4. **Test Independently**: Test your custom functions before integrating with the full training pipeline.

5. **Document Your Functions**: Add docstrings explaining the expected inputs and outputs.

6. **Consider Batching**: For expensive operations (like API calls), consider batch processing when possible.
